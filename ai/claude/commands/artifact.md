...
6. **Voice (only if `--voice`).** Invoke the `speak` skill with a ≤100-word summary of the artifact. Without `--voice`, return the spoken-text blurb as plain output so the user can trigger TTS themselves.
7. **Report.** Print the repo-relative path, an `open <path>` command, and whether it's durable or throwaway. Never suggest tracking it.

## Notes

- `docs/.ai/` is globally gitignored on purpose — local-only scratch space. Never stage, force-add, or commit artifacts there, and never suggest it. "Durable" means it survives on disk, not in git.
