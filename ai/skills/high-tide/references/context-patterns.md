...
- Use `scripts/context_percent.py` to normalize measured payloads or status snippets into a used percentage. In this dotfiles setup, Claude's statusline writes minimal fresh telemetry per session to `<config-dir>/statusline/by-session/<session-id>.json`, and the helper reads the caller's own file automatically when no stdin/file/input is provided. It will not fall back to another session's file; every pane on the host writes into the same few config dirs, so an unattributed reading is someone else's. The helper should reject unrelated percentages such as rate-limit usage and convert explicit remaining-context values into used-context values.
