...
render_vars() {
    ...
    [[ -n "$ts_if" ]] || {
        echo "✗ Could not detect Tailscale interface; pass --ts-if" >&2
        exit 1
    }
    [[ -n "$wan_if" ]] || {
        echo "✗ Could not detect WAN interface; pass --wan-if" >&2
        exit 1
    }
    [[ -n "$ts_cidr" ]] || {
        echo "✗ Could not detect Tailscale address; pass --ts-cidr" >&2
        exit 1
    }
    [[ -n "$lan_ip" ]] || {
        echo "✗ Could not detect LAN IP; pass --lan-ip" >&2
        exit 1
    }
...
cmd_open_updates() {
    ...
    if [[ $dur -gt 0 ]]; then
        (
            ...
            logger -t offline-firewall "Update window auto-closed after ${dur}s"
        ) &
        local pid=$!
